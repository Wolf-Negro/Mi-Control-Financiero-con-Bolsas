...
export default function Home() {
  ...
  const guardarRegistro = () => {
    if (monto <= 0) return alert("Pon un monto válido");
    let nuevoHistorial;
    const datosBase = { monto, nota: nota || 'Sin concepto', fecha, tipo, categoria: tipo === 'egreso' ? categoriaEgreso : undefined };
  ...
  const eliminar = (id: number) => {
    ...
    setHistorial(nuevo);
    localStorage.setItem('mis_finanzas_v3', JSON.stringify(nuevo));
  }
    ...
    <main className="min-h-screen bg-slate-50 p-5 pb-24 font-sans max-w-md mx-auto text-slate-900 leading-tight">
      <header className="flex justify-between items-center mb-6 pt-2">
        <div>
          ...
          <p className="text-xs font-bold text-slate-500 mt-1 uppercase">Saldo: <span className="text-blue-600 font-black">S/ {saldoGlobal.toFixed(2)}</span></p>
        </div>
        <div className="flex bg-white p-1 rounded-2xl shadow-sm border border-slate-200">
          <button onClick={() => setTipo('ingreso')} className={`px-4 py-2 rounded-xl text-[10px] font-black ${tipo === 'ingreso' ? 'bg-green-600 text-white shadow-md shadow-green-200' : 'text-slate-400'}`}>RECIBIDO</button>
          <button onClick={() => setTipo('egreso')} className={`px-4 py-2 rounded-xl text-[10px] font-black ${tipo === 'egreso' ? 'bg-red-600 text-white shadow-md shadow-red-200' : 'text-slate-400'}`}>PAGADO</button>
        </div>
      </header>

      {/* DASHBOARD DE BOLSAS (SCROLL) */}
      ...
      {/* FORMULARIO */}
      <section className="bg-white rounded-[2.5rem] p-7 shadow-xl shadow-blue-900/10 mb-6 border border-white space-y-5">
        ...
        <input type="text" value={nota} placeholder="¿Qué concepto es?" className="w-full text-base font-bold outline-none text-slate-700 bg-slate-50 p-4 rounded-2xl placeholder:text-slate-300" onChange={(e) => setNota(e.target.value)} />
      </section>
      ...
      <div className="space-y-4 mb-8">
        <div className="relative">
          <input type="text" placeholder="Buscar movimiento..." className="w-full bg-white p-4 pl-12 rounded-3xl text-sm font-bold shadow-sm border border-slate-100 outline-none focus:border-blue-400" onChange={(e) => setBusqueda(e.target.value)} />
          <span className="absolute left-4 top-4 text-slate-300 font-black">🔍</span>
        </div>
        <div className="flex justify-between items-center bg-blue-900 p-4 rounded-[2rem] shadow-lg text-white">
          <button onClick={() => setMesFiltro(m => m === 0 ? 11 : m - 1)} className="font-black text-xl px-2">‹</button>
          <p className="text-xs font-black uppercase tracking-widest">{meses[mesFiltro]} {anioFiltro}</p>
          <button onClick={() => setMesFiltro(m => m === 11 ? 0 : m + 1)} className="font-black text-xl px-2">›</button>
        </div>
      </div>
        ...
        {Object.entries(registrosAgrupados).map(([fechaGrupo, regs]) => (
          <div key={fechaGrupo}>
            <div className="flex items-center gap-3 mb-4">
              <span className="text-[11px] font-black text-slate-400 bg-slate-200 px-3 py-1 rounded-full uppercase tracking-tighter">{fechaGrupo}</span>
              <div className="h-[2px] bg-slate-200 flex-1 rounded-full"></div>
            </div>
            <div className="space-y-3">
              {regs.map(r => (
                <div key={r.id} onClick={() => { setEditandoId(r.id); setMonto(r.monto); setNota(r.nota); setFecha(r.fecha); setTipo(r.tipo); window.scrollTo({top:0, behavior:'smooth'}); }} className="bg-white p-5 rounded-[2.2rem] flex justify-between items-center shadow-sm border-2 border-transparent active:bg-blue-50 cursor-pointer group">
                  <div className="flex-1">
                    <p className="text-sm font-black text-slate-900 uppercase leading-none mb-1 group-hover:text-blue-700 transition-colors">{r.nota}</p>
                  ...
                  <div className="flex items-center gap-4">
                    <p className={`font-black text-lg tracking-tighter ${r.tipo === 'ingreso' ? 'text-green-600' : 'text-red-600'}`}>
                      {r.tipo === 'ingreso' ? '+' : '-'} S/ {r.monto.toFixed(2)}
                    </p>
                    <button onClick={(e) => { e.stopPropagation(); eliminar(r.id); }} className="text-slate-200 hover:text-red-500 font-bold text-lg p-2">✕</button>
                  </div>
                </div>
              ))}
            </div>
